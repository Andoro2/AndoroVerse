...
public class EnemyLifeController : MonoBehaviour
{
    ...
    public GameObject m_ExplosionDeath;
    ...
    public void TakeDamage(float Dmg, Vector3 KnockBackDir, float KnockBackForce)
    {
        ...
        if (m_LifePoints <= 0f)
        {
            ...
            GameObject ExploVFX = Instantiate(m_ExplosionDeath, transform);
            if (gameObject.transform.name == "Levitank") ExploVFX.transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
